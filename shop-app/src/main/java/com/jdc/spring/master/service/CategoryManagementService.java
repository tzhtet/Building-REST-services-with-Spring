...
import com.jdc.spring.controller.input.CategoryForm;
import com.jdc.spring.controller.input.CategorySearch;
import com.jdc.spring.controller.output.CategoryInfo;
import com.jdc.spring.master.entity.Category;
import com.jdc.spring.master.entity.Category_;
...
public class CategoryManagementService {
	...
	public void save(CategoryForm form) {
		...
		if(repo.countByNameIgnoreCase(form.getName()) == 0L) {
			...
			if(null != form.getId()) {
				repo.findById(form.getId()).ifPresent(enttiy -> enttiy.setName(form.getName()));
			}else {
				var entity = new Category();
