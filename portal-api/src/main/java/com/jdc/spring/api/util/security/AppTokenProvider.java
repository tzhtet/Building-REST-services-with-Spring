...
public class AppTokenProvider {
	...
	public String generateRereshToken(Authentication authentication) {
		return generate(Type.Refresh, authentication);
	}
	
	private Authentication parse(Type type, String token) {

		var jwt = Jwts.parser().requireIssuer(getIssuer(type)).verifyWith(secretKey).build().parseSignedClaims(token);
		...
		return UsernamePasswordAuthenticationToken.authenticated(username, null,
				Arrays.stream(roleValue.split(",")).map(a -> new SimpleGrantedAuthority(a)).toList());
	}
	...
	private String generate(Type type, Authentication authentication) {
		...
		calender.add(Calendar.SECOND, type == Type.Access ? accessLife : refreshLife);
		var expiration = calender.getTime();
